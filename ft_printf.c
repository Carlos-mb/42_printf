...
/*   Updated: 2026/01/10 00:07:48 by cmelero-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static char	ft_print_char(size_t *p_i, va_list args)
{
	char	c;
	int		i;
...
static char	ft_print_s(size_t *p_i, va_list args)
{
	char	*c;
	int		count;
...
static int	ft_write_hex(uintptr_t n, char *hex)
{
	int		count;
	...
	if (n >= 16)
	{
		count += ft_write_hex (n / 16, "0123456789abcdefgh");
...
static char	ft_print_p(size_t *p_i, va_list args)
{
	void		*p;
	uintptr_t	value;
	...
	value = (uintptr_t) p;
	(*p_i) += 1;
	return (write (1, "0x", 2) + ft_write_hex(value, "0123456789abcdefgh"));
}
...
static char	ft_print_d_i(size_t *p_i, va_list args)
{
	int		n;
	char	*c_value;
	int		count;
...
static char	ft_print_u(size_t *p_i, va_list args)
{
	unsigned int	n;
	char			*c_value;
	int				count;
...
static char	ft_print_x(size_t *p_i, va_list args, char *hex)
{
	unsigned	n;
